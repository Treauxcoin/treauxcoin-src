...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("TREAUXCOIN-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #TREAUXCOINTEST2\r");
            Send(hSocket, "WHO #TREAUXCOINTEST2\r");
        } else {
            // randomly join #TREAUXCOIN00-#TREAUXCOIN05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #TREAUXCOIN%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #TREAUXCOIN%02d\r", channel_number).c_str());
        }
