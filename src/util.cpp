...
#else
    const char* pszModule = "TREAUXCOIN";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TREAUXCOIN
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TREAUXCOIN
    // Mac: ~/Library/Application Support/TREAUXCOIN
    // Unix: ~/.TREAUXCOIN
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TREAUXCOIN";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "TREAUXCOIN.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "TREAUXCOINd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong TREAUXCOIN will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("TREAUXCOIN"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
